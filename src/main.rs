...
fn main() {
    ...
    if args.name.is_some() {
        ...
        let mut fileref = OpenOptions::new()
            .append(true)
            .open(format!(r"{}\Cargo.toml", botname))
...
dotenv_codegen = "0.15.0"
      "#.as_bytes()).expect("Failed to add dependencies");
        ...
        fs::create_dir(format!(r"{}\src\commands", botname))
            .expect("Failed creating the commands directory");
        println!("Succesfully created commands directory!");

        // Create mod.rs
        let mut file = fs::File::create(format!(r"{}\src\commands\mod.rs", botname))
            .expect("creating command file failed");

        // Fill main.rs
        let mainimport = Asset::get("def_com/main.rs").expect("Couldn't fetch main.rs file");
        let mainbytes = std::str::from_utf8(mainimport.data.as_ref())
            .expect("Couldn't convert main.rs to bytes");
        let mut mainfile = OpenOptions::new()
            .write(true)
            .truncate(true)
            .open(format!(r"{}\src\main.rs", botname))
            .expect("Unable to open main.rs");
        mainfile
            .write_all(mainbytes.as_bytes())
            .expect("Couldn't write to main.rs");
        ...
        let helpbytes = std::str::from_utf8(helpimport.data.as_ref())
            .expect("Couldn't convert help.rs to bytes");
        let mut helpfile = fs::File::create(format!(r"{}\src\commands\help.rs", botname))
            .expect("couldn't create help command file");
        helpfile
            .write_all(helpbytes.as_bytes())
            .expect("Couldn't create the help command");
        ...
        // Create .env file
        let mut envfile =
            fs::File::create(format!(r"{}\.env", botname)).expect("Couldn't create a .env file");
        envfile
            .write_all("DISCORD_TOKEN=".as_bytes())
